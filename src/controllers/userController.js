"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserController = void 0;
var prisma_1 = require("../utils/prisma");
var UserController = /** @class */ (function () {
    function UserController() {
    }
    // Obter dados de um usuário
    UserController.getUser = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var id, user, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        id = req.params.id;
                        return [4 /*yield*/, prisma_1.prisma.user.findUnique({
                                where: { id: id },
                                select: {
                                    id: true,
                                    name: true,
                                    username: true,
                                    email: true,
                                    profileImage: true,
                                    createdAt: true,
                                    tweets: {
                                        select: {
                                            id: true,
                                            content: true,
                                            createdAt: true,
                                            likes: {
                                                select: {
                                                    id: true,
                                                    userId: true
                                                }
                                            },
                                            replies: {
                                                select: {
                                                    id: true,
                                                    content: true,
                                                    createdAt: true,
                                                    author: {
                                                        select: {
                                                            id: true,
                                                            name: true,
                                                            username: true,
                                                            profileImage: true
                                                        }
                                                    }
                                                }
                                            },
                                            _count: {
                                                select: {
                                                    likes: true,
                                                    replies: true
                                                }
                                            }
                                        },
                                        orderBy: {
                                            createdAt: 'desc'
                                        }
                                    },
                                    followers: {
                                        select: {
                                            follower: {
                                                select: {
                                                    id: true,
                                                    name: true,
                                                    username: true,
                                                    profileImage: true
                                                }
                                            }
                                        }
                                    },
                                    following: {
                                        select: {
                                            following: {
                                                select: {
                                                    id: true,
                                                    name: true,
                                                    username: true,
                                                    profileImage: true
                                                }
                                            }
                                        }
                                    },
                                    _count: {
                                        select: {
                                            tweets: true,
                                            followers: true,
                                            following: true
                                        }
                                    }
                                }
                            })];
                    case 1:
                        user = _a.sent();
                        if (!user) {
                            res.status(404).json({ error: 'Usuário não encontrado' });
                            return [2 /*return*/];
                        }
                        res.json({
                            user: __assign(__assign({}, user), { followers: user.followers.map(function (f) { return f.follower; }), following: user.following.map(function (f) { return f.following; }) })
                        });
                        return [3 /*break*/, 3];
                    case 2:
                        error_1 = _a.sent();
                        console.error('Erro ao buscar usuário:', error_1);
                        res.status(500).json({ error: 'Erro interno do servidor' });
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    // Seguir um usuário
    UserController.followUser = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var followingId, followerId, userToFollow, existingFollow, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 4, , 5]);
                        followingId = req.params.id;
                        followerId = req.userId;
                        // Verificar se não está tentando seguir a si mesmo
                        if (followerId === followingId) {
                            res.status(400).json({ error: 'Você não pode seguir a si mesmo' });
                            return [2 /*return*/];
                        }
                        return [4 /*yield*/, prisma_1.prisma.user.findUnique({
                                where: { id: followingId }
                            })];
                    case 1:
                        userToFollow = _a.sent();
                        if (!userToFollow) {
                            res.status(404).json({ error: 'Usuário não encontrado' });
                            return [2 /*return*/];
                        }
                        return [4 /*yield*/, prisma_1.prisma.follow.findUnique({
                                where: {
                                    followerId_followingId: {
                                        followerId: followerId,
                                        followingId: followingId
                                    }
                                }
                            })];
                    case 2:
                        existingFollow = _a.sent();
                        if (existingFollow) {
                            res.status(409).json({ error: 'Você já está seguindo este usuário' });
                            return [2 /*return*/];
                        }
                        // Criar o relacionamento de seguir
                        return [4 /*yield*/, prisma_1.prisma.follow.create({
                                data: {
                                    followerId: followerId,
                                    followingId: followingId
                                }
                            })];
                    case 3:
                        // Criar o relacionamento de seguir
                        _a.sent();
                        res.status(201).json({ message: 'Usuário seguido com sucesso' });
                        return [3 /*break*/, 5];
                    case 4:
                        error_2 = _a.sent();
                        console.error('Erro ao seguir usuário:', error_2);
                        res.status(500).json({ error: 'Erro interno do servidor' });
                        return [3 /*break*/, 5];
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    // Deixar de seguir um usuário
    UserController.unfollowUser = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var followingId, followerId, existingFollow, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 3, , 4]);
                        followingId = req.params.id;
                        followerId = req.userId;
                        return [4 /*yield*/, prisma_1.prisma.follow.findUnique({
                                where: {
                                    followerId_followingId: {
                                        followerId: followerId,
                                        followingId: followingId
                                    }
                                }
                            })];
                    case 1:
                        existingFollow = _a.sent();
                        if (!existingFollow) {
                            res.status(404).json({ error: 'Você não está seguindo este usuário' });
                            return [2 /*return*/];
                        }
                        // Remover o relacionamento
                        return [4 /*yield*/, prisma_1.prisma.follow.delete({
                                where: {
                                    followerId_followingId: {
                                        followerId: followerId,
                                        followingId: followingId
                                    }
                                }
                            })];
                    case 2:
                        // Remover o relacionamento
                        _a.sent();
                        res.json({ message: 'Você deixou de seguir este usuário' });
                        return [3 /*break*/, 4];
                    case 3:
                        error_3 = _a.sent();
                        console.error('Erro ao deixar de seguir usuário:', error_3);
                        res.status(500).json({ error: 'Erro interno do servidor' });
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    return UserController;
}());
exports.UserController = UserController;
